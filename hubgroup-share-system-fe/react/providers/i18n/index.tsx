...
import { FederationAppInitProps, FederationDataObservableType } from '../../../types/federation.type';
...
const I18nProvider: React.FC<Props> = ({
    ...
    const federationResolver = useSelector((state: Record<"common", CommonReducerState>) => state.common.federationResolver);
    ...
    useEffect(() => {
        if (Object.keys(federationResolver).length > 0) {
            if (federationResolver.localeResources) {
                i18n.addResourceBundle(federationResolver.currentLanguageSelected, 'translation', federationResolver.localeResources);
                i18n.changeLanguage(federationResolver.currentLanguageSelected);
                // setResolveSuccess(true);
                // console.log(federationResolver);
            } else {
                // console.log(currentLanguage);
                if (currentLanguage) {
                    (async () => {
                        // Đoạn này 2 tháng trước xảy ra lỗi khi dispatch federationResolver nên chuyển qua async tại đây
                        const response = await commonService.onGetsLocaleResourcesByLanguage({
                        ...
                        if (model) {
                            // console.log(model);
                            i18n.addResourceBundle(currentLanguage, 'translation', model);
                            i18n.changeLanguage(currentLanguage);
                            // console.log(federationResolver.moduleLocaleKey + currentLanguage);
                            federationResolver.updateLocale(federationResolver.moduleLocaleKey, model);
                        }
                        // setResolveSuccess(true);
                    })();
                }
    ...
    useEffect(() => {
        ...
        if (Object.keys(federationResolver).length > 0) {
            if (federationResolver.data$) {
                subscribe = federationResolver.data$.subscribe((data: FederationDataObservableType) => {
                    // console.log(data);
                    // console.log(federationResolver);
                    if (data.type == DataObservableTypeEnum.ChangeLanguage) {
    ...
    // console.log(isResolveSuccess);
    // console.log(federationResolver.localeResources);
